...
class RedisClient {
  ...
  RedisClient._(String this.connectionString) {
    connectionFuture = RedisConnection.connect(connectionString).then((conn) => connection = conn);
  }
   ...
   * configuration file.
   */
  Future auth(String password) => connection.auth(password);
  ...
  Future flushdb() => connection.rawSend([RedisCommand.FLUSHDB]).receiveStatus(StatusReply.OK);

  /// Delete all the keys of all the existing databases, not just the currently
  /// selected one. This command never fails.
  Future flushall() => connection.rawSend([RedisCommand.FLUSHALL]).receiveStatus(StatusReply.OK);

  /// Returns the DateTime of the last DB [save] executed with success.
  Future<DateTime> get lastsave => connection
      .rawSend([RedisCommand.LASTSAVE])
      .receiveInteger()
      .then((int unixTs) => new DateTime.fromMillisecondsSinceEpoch(unixTs * 1000, isUtc: true));
   ...
   */
  Future save() => connection.rawSend([RedisCommand.SAVE]).receiveStatus(StatusReply.OK);
   ...
   */
  Future bgrewriteaof() => connection.rawSend([RedisCommand.BGREWRITEAOF]).receiveStatus(StatusReply.OK);
   ...
   */
  Future quit() => connection.rawSend([RedisCommand.QUIT]).receiveStatus(StatusReply.OK);
  ...
  Future<List<String>> keys(String pattern) =>
      connection.sendCommand(RedisCommand.KEYS, [pattern]).receiveMultiBulkStrings();
  ...
  Future<List<String>> mget(List<String> keys) =>
      keys.isEmpty ? new Future.value([]) : connection.sendCommand(RedisCommand.MGET, keys).receiveMultiBulkStrings();

  /// Sets the value of given key, and returns the value that was stored previously.
  Future<String> getset(String key, String value) =>
      connection.sendCommand(RedisCommand.GETSET, [key, value]).receiveBulkString();

  /// Sets the value of given key.
  Future set(String key, String value) =>
      connection.sendCommand(RedisCommand.SET, [key, value]).receiveStatus(StatusReply.OK);
  ...
  Future setex(String key, int expireInSecs, String value) =>
      connection.sendCommand(RedisCommand.SETEX, [key, expireInSecs.toString(), value]).receiveStatus(StatusReply.OK);
  ...
  Future psetex(String key, int expireInMs, String value) =>
      connection.sendCommand(RedisCommand.PSETEX, [key, expireInMs.toString(), value]).receiveStatus(StatusReply.OK);
  ...
  Future mset(Map<String, String> map) =>
      connection.sendCommand(RedisCommand.MSET, serializer.serializeToList(map)).receiveStatus(StatusReply.OK);
  ...
  Future<bool> msetnx(Map<String, String> map) =>
      connection.sendCommand(RedisCommand.MSETNX, serializer.serializeToList(map)).receiveBool();
  ...
  Future<int> mdel(List<String> keys) =>
      keys.isEmpty ? new Future.value(0) : connection.sendCommand(RedisCommand.DEL, keys).receiveInteger();
  ...
  Future<int> incrby(String key, int count) =>
      connection.sendCommand(RedisCommand.INCRBY, [key, count.toString()]).receiveInteger();
  ...
  Future<double> incrbyfloat(String key, double count) =>
      connection.sendCommand(RedisCommand.INCRBYFLOAT, [key, count.toString()]).receiveDouble();
  ...
  Future<int> decrby(String key, int count) =>
      connection.sendCommand(RedisCommand.DECRBY, [key, count.toString()]).receiveInteger();
  ...
  Future<int> append(String key, String value) =>
      connection.sendCommand(RedisCommand.APPEND, [key, value]).receiveInteger();
  ...
  /**
     ...
     */
  Future<String> getrange(String key, int fromIndex, int toIndex) => connection
      .sendCommand(RedisCommand.GETRANGE, [key, fromIndex.toString(), toIndex.toString()]).receiveBulkString();
  ...
  Future<int> setrange(String key, int offset, String value) =>
      connection.sendCommand(RedisCommand.SETRANGE, [key, offset.toString(), value]).receiveInteger();
  ...
  Future<int> getbit(String key, int offset) =>
      connection.sendCommand(RedisCommand.GETBIT, [key, offset.toString()]).receiveInteger();
  ...
  Future<int> setbit(String key, int offset, int value) =>
      connection.sendCommand(RedisCommand.SETBIT, [key, offset.toString(), value.toString()]).receiveInteger();

  /**
     *  Returns a random key from the currently selected database. Or nil when
     *  the database is empty.
     */
  Future<String> randomkey() => connection.sendCommand(RedisCommand.RANDOMKEY, []).receiveBulkString();
  ...
  Future<String> rename(String oldKey, String newKey) =>
      connection.sendCommand(RedisCommand.RENAME, [oldKey, newKey]).receiveStatus('OK');
  ...
  Future<bool> renamenx(String oldKey, String newKey) =>
      connection.sendCommand(RedisCommand.RENAMENX, [oldKey, newKey]).receiveBool();
  ...
  /**
     ...
     */
  Future<bool> expireat(String key, DateTime date) => connection.sendCommand(
      RedisCommand.EXPIREAT, [key, (date.toUtc().millisecondsSinceEpoch ~/ 1000).toString()]).receiveBool();
  ...
  Future<bool> pexpire(String key, int expireInMs) =>
      connection.sendCommand(RedisCommand.PEXPIRE, [key, serializer.serializeToString(expireInMs)]).receiveBool();
   ...
   */
  Future<bool> pexpireat(String key, DateTime date) => connection.sendCommand(
      RedisCommand.PEXPIREAT, [key, (date.toUtc().millisecondsSinceEpoch ~/ 1000).toString()]).receiveBool();
   ...
   */
  Future<int> pttl(String key) => connection.sendCommand(RedisCommand.PTTL, [key]).receiveInteger();
   ...
   */
  Future watch(List<String> keys) => keys.isEmpty
      ? new Future.value(StatusReply.OK)
      : connection.sendCommand(RedisCommand.WATCH, keys).receiveStatus(StatusReply.OK);

  /**
   * Marks the given keys to be watched for conditional execution of a transaction.
   */
  Future unwatch() => connection.rawSend([RedisCommand.UNWATCH]).receiveStatus(StatusReply.OK);

  /*
   * Marks the start of a transaction block. Subsequent commands will be queued for
   * atomic execution using EXEC.
   */
  Future multi() => connection.rawSend([RedisCommand.MULTI]).receiveStatus(StatusReply.OK);
   ...
   * connection state to normal.
   */
  Future discard() => connection.rawSend([RedisCommand.DISCARD]).receiveStatus(StatusReply.OK);
  ...
  Future<Set<Object>> smembers(String setId) =>
      connection.sendCommand(RedisCommand.SMEMBERS, [setId]).receiveMultiBulkSetDeserialized(serializer);
   ...
   */
  Future<int> sadd(String setId, Object value) => connection
      .sendCommandWithVariadicValues(RedisCommand.SADD, [setId], serializer.serializeToList(value))
      .receiveInteger();
   ...
   * An error is returned when the value stored at key is not a set.
   */
  Future<int> srem(String setId, Object value) => connection
      .sendCommandWithVariadicValues(RedisCommand.SREM, [setId], serializer.serializeToList(value))
      .receiveInteger();
  ...
  Future<Object> spop(String setId) =>
      connection.sendCommand(RedisCommand.SPOP, [setId]).receiveBulkDeserialized(serializer);
   ...
   */
  Future<bool> smove(String fromSetId, String toSetId, Object value) => connection
      .sendCommand(RedisCommand.SMOVE, [fromSetId, toSetId, serializer.serializeToString(value)]).receiveBool();
  ...
  Future<bool> sismember(String setId, Object value) =>
      connection.sendCommand(RedisCommand.SISMEMBER, [setId, serializer.serializeToString(value)]).receiveBool();
   ...
   */
  Future<Set<Object>> sinter(List<String> setIds) => connection
      .sendCommand(RedisCommand.SINTER, serializer.serializeToList(setIds))
   ...
   * is overwritten.
   */
  Future<int> sinterstore(String destination, List<String> setIds) => connection
      .sendCommandWithVariadicValues(RedisCommand.SINTERSTORE, [destination], serializer.serializeToList(setIds))
      .receiveInteger();
   ...
   */
  Future<Set<Object>> sunion(List<String> setIds) => connection
      .sendCommand(RedisCommand.SUNION, serializer.serializeToList(setIds))
      .receiveMultiBulkSetDeserialized(serializer);
   ...
   */
  Future<int> sunionstore(String destination, List<String> setIds) => connection
      .sendCommandWithVariadicValues(RedisCommand.SUNIONSTORE, [destination], serializer.serializeToList(setIds))
      .receiveInteger();
   ...
   * Keys that do not exist are considered to be empty sets.
   */
  Future<Set<Object>> sdiff(String fromSetId, List<String> withSetIds) => connection
      .sendCommandWithVariadicValues(RedisCommand.SDIFF, [fromSetId], serializer.serializeToList(withSetIds))
      .receiveMultiBulkSetDeserialized(serializer);
  ...
  Future<int> sdiffstore(String destination, List<String> setIds) => connection
      .sendCommandWithVariadicValues(RedisCommand.SDIFFSTORE, [destination], serializer.serializeToList(setIds))
      .receiveInteger();
  ...
  Future<dynamic> srandmember(String setId, [int count]) {
    if (count == null) {
      return connection.sendCommand(RedisCommand.SRANDMEMBER, [setId]).receiveBulkDeserialized(serializer);
    } else {
      return connection.sendCommand(RedisCommand.SRANDMEMBER,
          [setId, serializer.serializeToString(count)]).receiveMultiBulkSetDeserialized(serializer);
    }
  }
  ...
  Future<dynamic> sort(String key,
      {String by, int skip, int take, List<String> get, bool desc: false, bool alpha: false, String destination}) {
    ...
    return connection.sendCommand(RedisCommand.SORT, values).receiveMultiBulkDeserialized(serializer);
  }
   ...
   * returned.
   */
  Future<int> lpush(String key, Object value) => connection
      .sendCommandWithVariadicValues(RedisCommand.LPUSH, [key], serializer.serializeToList(value))
      .receiveInteger();
   ...
   * returned.
   */
  Future<List<Object>> lrange(String key, {int startingFrom: 0, int endingAt: -1}) => connection.sendCommand(
      RedisCommand.LRANGE,
      [key, startingFrom.toString(), endingAt.toString()]).receiveMultiBulkDeserialized(serializer);
  ...
  Future<int> lpushx(String key, Object value) =>
      connection.sendCommand(RedisCommand.LPUSHX, [key, serializer.serializeToString(value)]).receiveInteger();
   ...
   * returned.
   */
  Future<int> rpush(String key, Object value) => connection
      .sendCommandWithVariadicValues(RedisCommand.RPUSH, [key], serializer.serializeToList(value))
      .receiveInteger();
  ...
  Future<int> rpushx(String key, Object value) =>
      connection.sendCommand(RedisCommand.RPUSHX, [key, serializer.serializeToString(value)]).receiveInteger();
  ...
  Future<String> ltrim(String key, int start, int end) =>
      connection.sendCommand(RedisCommand.LTRIM, [key, start.toString(), end.toString()]).receiveStatus('OK');
   ...
   */
  Future<int> lrem(String key, int count, Object value) => connection
      .sendCommand(RedisCommand.LREM, [key, count.toString(), serializer.serializeToString(value)]).receiveInteger();
   ...
   */
  Future<int> llen(String key) => connection.sendCommand(RedisCommand.LLEN, [key]).receiveInteger();
  ...
  Future<Object> lindex(String key, int index) =>
      connection.sendCommand(RedisCommand.LINDEX, [key, index.toString()]).receiveBulkDeserialized(serializer);
   ...
   */
  Future<int> linsert(String key, String position, Object pivot, Object value) => connection.sendCommand(
      RedisCommand.LINSERT,
      [key, position, serializer.serializeToString(pivot), serializer.serializeToString(value)]).receiveInteger();
   ...
   */
  Future<String> lset(String key, int index, Object value) => connection
      .sendCommand(RedisCommand.LSET, [key, index.toString(), serializer.serializeToString(value)]).receiveStatus('OK');

  /**
   * Returns the value of the first element, or nil when key does not exist.
   */
  Future<Object> lpop(String key) =>
      connection.sendCommand(RedisCommand.LPOP, [key]).receiveBulkDeserialized(serializer);

  /**
   * Returns the value of the last element, or nil when key does not exist.
   */
  Future<Object> rpop(String key) =>
      connection.sendCommand(RedisCommand.RPOP, [key]).receiveBulkDeserialized(serializer);
  ...
  Future<Object> rpoplpush(String source, String destination) =>
      connection.sendCommand(RedisCommand.RPOPLPUSH, [source, destination]).receiveBulkDeserialized(serializer);
  ...
  Future blpop(Object key, {timeout: 0}) {
    ...
    return connection.sendCommand(RedisCommand.BLPOP, values).receiveMultiBulkMapDeserialized(serializer);
  }
  ...
  Future brpop(Object key, {timeout: 0}) {
    var values = serializer.serializeToList(key);
    values.add(timeout.toString());
    return connection.sendCommand(RedisCommand.BRPOP, values).receiveMultiBulkMapDeserialized(serializer);
  }
  ...
  Future<Object> brpoplpush(String source, String destination, {timeout: 0}) =>
      connection.sendCommand(RedisCommand.BRPOPLPUSH, [
        ...
        serializer.serializeToString(destination),
        timeout.toString()
   ...
   * returned.
   */
  Future<int> zadd(Object setId, Iterable<ZSetEntry> zset) => connection
      .sendCommandWithVariadicValues(
          RedisCommand.ZADD, [serializer.serializeToString(setId)], serializer.serializeFromZSet(zset))
      .receiveInteger();
   ...
   */
  Future<int> zsadd(Object setId, num score, Object value) => connection.sendCommand(RedisCommand.ZADD, [
        serializer.serializeToString(setId),
        serializer.serializeToString(score),
   ...
   * non existing members.
   */
  Future<int> zsrem(Object setId, Object value) => connection.sendCommand(
      RedisCommand.ZREM, [serializer.serializeToString(setId), serializer.serializeToString(value)]).receiveInteger();

  /**
   * Returns the number of members removed from the sorted set, not including
   * non existing members.
   */
  Future<int> zmrem(Object setId, Iterable value) => connection
      .sendCommandWithVariadicValues(
          RedisCommand.ZREM, [serializer.serializeToString(setId)], serializer.serializeToList(value))
      .receiveInteger();
   ...
   */
  Future<double> zincrby(String setId, num increment, Object value) => connection.sendCommand(
      RedisCommand.ZINCRBY, [setId, increment.toString(), serializer.serializeToString(value)]).receiveDouble();
  ...
  Future<int> zrank(String setId, Object value) =>
      connection.sendCommand(RedisCommand.ZRANK, [setId, serializer.serializeToString(value)]).receiveInteger();
  ...
  Future<int> zrevrank(String setId, Object value) =>
      connection.sendCommand(RedisCommand.ZREVRANK, [setId, serializer.serializeToString(value)]).receiveInteger();
   ...
   * Read more at: http://redis.io/commands/zrange
   */
  Future<dynamic> zrevrange(String setId, int min, int max, {bool withScores: false}) {
    return withScores
        ? connection.sendCommand(RedisCommand.ZREVRANGE,
            [setId, min.toString(), max.toString(), 'WITHSCORES']).receiveMultiBulkMapDeserialized(serializer)
        : connection.sendCommand(RedisCommand.ZREVRANGE,
            [setId, min.toString(), max.toString()]).receiveMultiBulkSetDeserialized(serializer);
  }
   ...
   */
  Future<dynamic> zrangebyscore(String setId,
      {num min,
      num max,
    ...
    if (withScores && hasLimit) {
      return connection.sendCommand(RedisCommand.ZRANGEBYSCORE, [
        setId,
        _setMin(min, minExclusive),
        _setMax(max, maxExclusive),
        'WITHSCORES',
        'LIMIT',
        offsetString,
        countString
    ...
    return connection.sendCommand(RedisCommand.ZRANGEBYSCORE,
        [setId, _setMin(min, minExclusive), _setMax(max, maxExclusive)]).receiveMultiBulkSetDeserialized(serializer);
  }
   ...
   */
  Future<dynamic> zrevrangebyscore(String setId,
      {num min,
      num max,
      bool minExclusive: false,
      bool maxExclusive: false,
  ...
  Future<int> zremrangebyrank(String setId, int min, int max) =>
      connection.sendCommand(RedisCommand.ZREMRANGEBYRANK, [setId, min.toString(), max.toString()]).receiveInteger();
   ...
   */
  Future<int> zremrangebyscore(String setId, {num min, num max, bool minExclusive: false, bool maxExclusive: false}) {
    return connection.sendCommand(RedisCommand.ZREMRANGEBYSCORE,
        [setId, _setMin(min, minExclusive), _setMax(max, maxExclusive)]).receiveInteger();
  }
  ...
  * does not exist.
  */
  Future<int> zcard(String setId) => connection.sendCommand(RedisCommand.ZCARD, [setId]).receiveInteger();
  ...
  Future<double> zscore(String setId, Object value) =>
      connection.sendCommand(RedisCommand.ZSCORE, [setId, serializer.serializeToString(value)]).receiveDouble();
  ...
  Future<int> zunionstore(String destination, List<String> setIds, {List<int> weights, String aggregate}) {
    ...
    return connection
        .sendCommandWithVariadicValues(RedisCommand.ZUNIONSTORE, [destination, numkeys], values)
        .receiveInteger();
  }
  ...
  Future<int> zinterstore(String destination, List<String> setIds, {List<String> weights, String aggregate}) {
    ...
    return connection
        .sendCommandWithVariadicValues(RedisCommand.ZINTERSTORE, [destination, numkeys], values)
        .receiveInteger();
  }
   ...
   */
  Future<bool> hsetnx(String hashId, String key, Object value) => connection
      .sendCommandWithVariadicValues(RedisCommand.HSETNX, [key], serializer.serializeToList(value))
      .receiveBool();
   ...
   */
  Future hmset(String hashId, Map<String, Object> map) => connection
      .sendCommandWithVariadicValues(RedisCommand.HMSET, [hashId], serializer.serializeToList(map))
      .receiveStatus('OK');
  ...
  Future<int> hincrby(String hashId, String key, int incrBy) =>
      connection.sendCommand(RedisCommand.HINCRBY, [hashId, key, incrBy.toString()]).receiveInteger();
  ...
  Future<double> hincrbyfloat(String hashId, String key, double incrBy) =>
      connection.sendCommand(RedisCommand.HINCRBYFLOAT, [hashId, key, incrBy.toString()]).receiveDouble();
  ...
  Future<Object> hget(String hashId, String key) =>
      connection.sendCommand(RedisCommand.HGET, [hashId, key]).receiveBulkDeserialized(serializer);
   ...
   */
  Future<List<Object>> hmget(String hashId, List<Object> keys) => connection
      .sendCommandWithVariadicValues(RedisCommand.HMGET, [hashId], serializer.serializeToList(keys))
  ...
  Future<int> hdel(String hashId, String key) =>
      connection.sendCommand(RedisCommand.HDEL, [hashId, key]).receiveInteger();
  ...
  Future<bool> hexists(String hashId, String field) =>
      connection.sendCommand(RedisCommand.HEXISTS, [hashId, field]).receiveBool();

  /**
   * Returns the number of fields contained in the hash stored at key
   * or 0 when key does not exist.
   */
  Future<int> hlen(String hashId) => connection.sendCommand(RedisCommand.HLEN, [hashId]).receiveInteger();

  /**
   * Returns list of fields in the hash, or an empty list when key does not exist.
   */
  Future<List<String>> hkeys(String hashId) =>
      connection.sendCommand(RedisCommand.HKEYS, [hashId]).receiveMultiBulkDeserialized(serializer);
  ...
  Future<List<Object>> hvals(String hashId) =>
      connection.sendCommand(RedisCommand.HVALS, [hashId]).receiveMultiBulkDeserialized(serializer);
  ...
  Future<Map<String, Object>> hgetall(String hashId) =>
      connection.sendCommand(RedisCommand.HGETALL, [hashId]).receiveMultiBulkMapDeserialized(serializer);
  ...
  Future publish(String channel, String message) =>
      connection.sendCommand(RedisCommand.PUBLISH, [channel, message]).receiveInteger();
}
